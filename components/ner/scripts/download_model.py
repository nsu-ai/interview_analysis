...
def download_ner() -> bool:
    ...
    model_logger.info(f'Model URL is {final_url}')
    ...
    if (total_size_in_bytes != 0) and (progress_bar.n != total_size_in_bytes):
        return False
    model_logger.info(f'Model is downloaded into {zip_archive_name}.')
    with zipfile.ZipFile(zip_archive_name) as archive:
        archive.extractall(model_path)
    os.remove(zip_archive_name)
    model_logger.info(f'Model is unzipped into {model_path}.')
    return True
